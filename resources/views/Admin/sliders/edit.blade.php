        ...
        <div class="row">
            <div class="col-md-8">
                ...
                <div class="m-portlet m-portlet--tab">
                    ...
                    <!--begin::Form-->
                    <form action="{{route('sliders.update' , ['slider'=>$slider->id])}}" method="post" id="create"  enctype="multipart/form-data" class="m-form m-form--fit m-form--label-align-right">
                        {{csrf_field()}}
                        {{method_field('PATCH')}}
                        ...
                        <div class="m-portlet__body " data-code-preview="true" data-code-html="true" data-code-js="false">
                            ...
                            <div class="form-group m-form__group">
                                ...
                                <input type="text" class="form-control m-input" id="title" aria-describedby="titleHelp" placeholder="" name="title" value="{{$slider->title}}">
                                <span class="m-form__help">{{__('admin.help_title')}}</span>
                            </div>
                            <div class="form-group m-form__group">
                                <label for="">{{__('admin.Description')}} </label>
                                <textarea class="form-control m-input description" id="description" name="description"  rows="5">{{$slider->description}}</textarea>
                            </div>
                            <div class="form-group m-form__group">
                                <label for="">{{__('admin.Link')}}</label>
                                <input type="text" class="form-control m-input" id="link" aria-describedby="titleHelp" placeholder="" name="link" value="{{$slider->link}}">
                            </div>
                            <div class="form-group m-form__group">
                                <label for="">{{__('admin.Order')}}</label>
                                <input type="text" class="form-control m-input" id="order" aria-describedby="titleHelp" placeholder="" name="order" value="{{$slider->order}}">
                            </div>
                            <div class="form-group m-form__group">
                                <label for="exampleSelect2">{{__('admin.Language')}}</label>
                                <select  class="form-control m-input " name="lang"  id="exampleSelect2">
                                    <option {{$slider->lang  == "fa" ? "selected" : ""}} value="fa">فارسی</option>
                                    <option {{$slider->lang  == "en" ? "selected" : ""}} value="en">english</option>
                                </select>
                            </div>
