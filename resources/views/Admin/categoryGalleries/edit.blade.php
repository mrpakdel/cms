        ...
        <div class="d-flex align-items-center">
            <div class="mr-auto">
                ...
                <ul class="m-subheader__breadcrumbs m-nav m-nav--inline">
                    ...
                    <li class="m-nav__item">
                        <a href="{{route('categoriesGallery.index')}}" class="m-nav__link">
                            <span class="m-nav__link-text">
                                {{__('admin.Archive')}}
                            </span>
                        </a>
        ...
        <div class="row">
            <div class="col-md-8">
                ...
                <div class="m-portlet m-portlet--tab">
                    ...
                    <!--begin::Form-->
                    <form action="{{route('categoriesGallery.update' , ['categoriesGallery'=>$categoriesGallery->slug])}}" method="post" id="create"  enctype="multipart/form-data" class="m-form m-form--fit m-form--label-align-right">
                        {{csrf_field()}}
                        {{method_field('PATCH')}}
                        ...
                        <div class="m-portlet__body " data-code-preview="true" data-code-html="true" data-code-js="false">
                            ...
                            <div class="form-group m-form__group">
                                ...
                                <input type="text" class="form-control m-input" id="title" aria-describedby="titleHelp" placeholder="" name="title" value="{{$categoriesGallery->title}}">
                            ...
                            <div class="form-group m-form__group">
                                ...
                                <select  class="form-control m-input " name="parent_id"  id="exampleSelect2">
                                    ...
                                    @foreach($categories as $category)
                                        <option value="{{$category->id}}" {{$category->id == $categoriesGallery->parent_id ? "selected" : ""}}>{{$category->title}}</option>
                                    @endforeach
                                </select>
                            </div>
                            <div class="form-group m-form__group">
                                <label for="">{{__('admin.Description')}} </label>
                                <textarea class="form-control m-input body" id="body" name="description"  rows="5">{{$categoriesGallery->description}}</textarea>
                            </div>
                            <div class="form-group m-form__group">
                                <label for="exampleSelect2">{{__('admin.Language')}}</label>
                                <select  class="form-control m-input " name="lang"  id="exampleSelect2">
                                    <option {{$categoriesGallery->lang == "fa" ? "selected":""}} value="fa">فارسی</option>
                                    <option {{$categoriesGallery->lang == "en" ? "selected":""}} value="en">English</option>
                                </select>
                            </div>
                            <div class="form-group m-form__group row">
                                <label class="col-3 col-form-label text-left" for="exampleSelect2">{{__('admin.Status')}}</label>
                                <span class="m-switch m-switch--lg m-switch--icon">
                                <label>
                                    <input type="checkbox" checked="checked" name="status" {{$categoriesGallery->status == 0 ? "checked" : ""}}>
                                    <span></span>
                                </label>
                                </span>
                            </div>
                    ...
                    <div class="m--padding-5">
                        <form  method="post" action="{{route('dropzone.store')}}" accept-charset="UTF-8" enctype="multipart/form-data" class="dropzone dz-clickable m-dropzone dropzone m-dropzone--success" id="image-upload">
                            ...
                            <div class="img-thumbnail">
                                <img src="{{$categoriesGallery->image}}" class="img-responsive img-thumbnail" alt="">
                            </div>
