        ...
        <div class="row">
            <div class="col-md-8">
                ...
                <div class="m-portlet m-portlet--tab">
                    ...
                    <!--begin::Form-->
                    <form action="{{route('gallery.store')}}" method="post" id="create"  enctype="multipart/form-data" class="m-form m-form--fit m-form--label-align-right">
                        {{csrf_field()}}
                        ...
                        <div class="m-portlet__body " data-code-preview="true" data-code-html="true" data-code-js="false">
                            ...
                            <div class="form-group m-form__group">
                                ...
                                <input type="text" class="form-control m-input" id="title" aria-describedby="titleHelp" placeholder="" name="title" value="{{$gallery->title}}">
                            ...
                            <div class="form-group m-form__group">
                                ...
                                <select  class="form-control m-input " name="category_gallery_id"  id="exampleSelect2">
                                    ...
                                    @foreach($albums as $album)
                                        <option value="{{$album->id}}" {{$album->id == $gallery->category_gallery_id ? "selected" : ""}}>{{$album->title}}</option>
                                    @endforeach
                                </select>
                            </div>
                            <div class="form-group m-form__group">
                                ...
                                <select  class="form-control m-input " name="type"  id="exampleSelect2">
                                    <option  value="0">{{__('admin.Select')}}</option>
                                    <option {{$gallery->type == 1 ? "selected" : ""}} value="1">{{__('admin.Sound')}}</option>
                                    <option {{$gallery->type == 2 ? "selected" : ""}} value="2">{{__('admin.Video')}}</option>
                                    <option {{$gallery->type == 3 ? "selected" : ""}} value="3">{{__('admin.Image')}}</option>
                                </select>
                            </div>
                            <div class="form-group m-form__group">
                                <label for="">{{__('admin.Order')}}</label>
                                <input type="text" class="form-control m-input" id="arrangement" aria-describedby="titleHelp" placeholder="" name="arrangement" value="{{$gallery->arrangement}}">
                                <span class="m-form__help">{{__('admin.help_title')}}</span>
                            </div>
                        </div>
                    ...
                    <div class="m--padding-5">
                        <form  method="post" action="{{route('file.store')}}" accept-charset="UTF-8" enctype="multipart/form-data" class="dropzone dz-clickable m-dropzone dropzone m-dropzone--success" id="image-upload">
                            ...
                            <div class="img-thumbnail">
                                <img src="{{$gallery->image}}" class="img-responsive img-thumbnail" alt="">
                            </div>
