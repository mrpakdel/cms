        ...
        <div class="row">
            ...
            <div class="col-md-8">
                ...
                <div class="m-portlet m-portlet--tab">
                    ...
                    <!--begin::Form-->
                    <form action="{{route('categories.update',['category'=>$category->slug])}}" method="post" id="create"  enctype="multipart/form-data" class="m-form m-form--fit m-form--label-align-right">
                        {{csrf_field()}}
                        {{method_field('PATCH')}}
                        ...
                        <div class="m-portlet__body">
                            ...
                            <div class="form-group m-form__group">
                                ...
                                <input type="text" class="form-control m-input" id="title" aria-describedby="titleHelp" placeholder="" name="title" value="{{$category->title}}">
                            ...
                            <div class="form-group m-form__group">
                                ...
                                <select  class="form-control m-input " name="parent_id"  id="exampleSelect2">
                                    @foreach($parents as $parent)
                                        <option value="{{$parent->id}}" {{$parent->id == $category->id ? "selected" : ""}}>{{$parent->title}}</option>
                                    @endforeach
                                </select>
                            </div>
                            <div class="form-group m-form__group">
                                <label for="">{{__('admin.Description')}} </label>
                                <textarea class="form-control m-input body" id="body" name="body"  rows="5">{{$category->body}}</textarea>
                            </div>
                            <div class="form-group m-form__group">
                                <label for="exampleSelect2">{{__('admin.Language')}}</label>
                                <select  class="form-control m-input " name="lang"  id="exampleSelect2">
                                    <option {{$category->lang == "fa" ? "selected":""}} value="fa">فارسی</option>
                                    <option {{$category->lang == "en" ? "selected":""}} value="en">English</option>
                                </select>
                            </div>
                        </div>
                        ...
                        <form  method="post" action="{{route('dropzone.store')}}" accept-charset="UTF-8" enctype="multipart/form-data" class="dropzone dz-clickable m-dropzone dropzone m-dropzone--success" id="image-upload">
                            ...
                            <div class="m-portlet__body">
                                ...
                                <div class="img-thumbnail">
                                    <img src="{{$category->image}}" class="img-responsive img-thumbnail" alt="">
                                </div>
                                <div class="dz-default dz-message">
                                    <span>{{__('admin.ClickToUploadImage')}}</span>
                                </div>
                                <input type="hidden" name="directory" value="category">
